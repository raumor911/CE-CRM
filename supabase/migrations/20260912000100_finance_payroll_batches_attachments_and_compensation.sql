...
create index if not exists idx_fin_payroll_batches_period_range
  on public.fin_payroll_batches(period_start, period_end);

create index if not exists idx_fin_payroll_batches_status
  on public.fin_payroll_batches(status);

create index if not exists idx_fin_payroll_batches_expense
  on public.fin_payroll_batches(financial_expense_id);
...
create index if not exists idx_fin_payroll_batch_items_period
  on public.fin_payroll_batch_items(payroll_period_id);
...
create index if not exists idx_fin_payroll_comp_history_profile
  on public.fin_payroll_compensation_history(payroll_profile_id, effective_from desc);

create unique index if not exists uq_fin_payroll_comp_history_active
  on public.fin_payroll_compensation_history(payroll_profile_id)
...
select
  ...
  profile.end_date,
  profile.periodicity,
  profile.current_period_amount,
  profile.estimated_monthly_cost,
  coalesce(profile.notes, 'Backfill inicial'),
  profile.created_by,
  coalesce(profile.created_at, timezone('utc', now()))
from public.fin_payroll_profiles profile
where not exists (
  select 1
  from public.fin_payroll_compensation_history history
  where history.payroll_profile_id = profile.id
);
...
create index if not exists idx_fin_expense_attachments_expense
  on public.fin_expense_attachments(expense_id, created_at);
...
values (
  ...
  true,
  35,
  'Salida real de caja por pago consolidado o individual de nomina.'
)
...
create policy if not exists "finance_documents_select_authenticated"
on storage.objects
for select
to authenticated
using (bucket_id = 'finance-documents');

create policy if not exists "finance_documents_insert_authenticated"
on storage.objects
for insert
to authenticated
with check (bucket_id = 'finance-documents');

create policy if not exists "finance_documents_delete_authenticated"
on storage.objects
for delete
to authenticated
using (bucket_id = 'finance-documents');
